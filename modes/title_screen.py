...
# Items du menu — dans l'ordre d'affichage
ITEMS = [
...
def run_title_screen(screen: pygame.Surface, clock: pygame.time.Clock, save: dict):
    """
    Boucle principale de l'écran titre.
    Retourne ("play", save) ou (None, save) si le joueur quitte.
    """
    w, h = screen.get_size()
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    ...
                    result = _handle_action(ITEMS[hov_idx]["action"], screen, clock, save, w, h, bg)
                    if result == "play":
                        return "play", save
                    elif result == "quit":
                        return None, save
        ...
        # RENDU
        screen.blit(bg, (0, 0)) if bg else screen.fill(theme.DARK)
        screen.blit(theme.make_vignette(w, h), (0, 0))

        # Zone titre en haut à gauche
        tx = int(w * 0.04)
        ty = int(h * 0.05)
        ...
        # Blason centré au-dessus du titre
        if blason:
            bx = tx + (title_w - blason.get_width()) // 2
            screen.blit(blason, (bx, ty))
            ty += blason.get_height() + int(h * 0.005)

        theme.draw_gold_rule(screen, tx, ty, theme.RULE_W)
        ty += 4

        # Titre bicolore : "Hexa" crème + "Hold" doré
        shadow = f_title.render("HexaHold", True, (0, 0, 0))
        ...
        slogan = f_slog.render("Défends les murs - Protège l'humanité", True, theme.GOLD_DIM)
        ...
        for i, item in enumerate(ITEMS):
            ...
            ix       = tx
            hit_rect = pygame.Rect(ix - 4, iy - 4, lbl.get_width() + 120, lbl.get_height() + 8)

            if hit_rect.collidepoint(mx, my):
                hov_idx = i
                if clicked:
                    result = _handle_action(item["action"], screen, clock, save, w, h, bg)
                    if result == "play":
                        return "play", save
                    elif result == "quit":
                        return None, save
            ...
            if is_act:
                ...
                # Barre verticale dorée à gauche de l'item actif
...
def _handle_action(action, screen, clock, save, w, h, bg):
    """Lance l'action correspondante — options ouvre un popup, play/quit retournent leur valeur."""
    if action == "options":
...
def _run_options(screen, clock, save, w, h, bg):
    """
    Popup Options avec deux sliders : volume musique et volume sons.
    Les valeurs sont appliquées en temps réel et sauvegardées à la fermeture.
    """
    ...
    while True:
        ...
        # On applique le volume musique en temps réel pour que le joueur entende l'effet
...
def _slider(screen, f_small, mx, my, clicked, rect, value):
    ...
    Le clic n'est pris en compte que si la souris est sur la zone élargie du slider.
    """
...
def _get_bg(w, h):
    """Charge et scale le fond d'écran titre — retourne None si l'asset est manquant."""
    bg = theme.load_img(theme.IMG_TITLE_BG)
    return pygame.transform.smoothscale(bg, (w, h)) if bg else None


def _get_blason(h):
    """Charge le blason redimensionné à 9% de la hauteur d'écran."""
...
def _draw_item_bg(screen, rect):
    """Fond parchemin dégradé derrière l'item actif — s'estompe vers la droite."""
...
def _draw_glow_text(screen, lbl, x, y, tick):
    """Texte doré avec lueur orange pulsante — l'intensité suit un sinus sur tick."""
    ...
    # Ombre portée sous le texte pour le détacher du fond
