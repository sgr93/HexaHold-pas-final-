"""
modes/histoire_widgets.py

Composants UI du mode histoire : Popup, ChapterPoint, Cinematic, Notification.
Chaque classe gère son propre état et son propre rendu — l'écran histoire
n'a qu'à les instancier et appeler update/draw à chaque frame.
"""
...
# Icône cadenas chargée une seule fois et mise en cache — pas envie de recharger à chaque frame
_LOCK_ICON = None


def _get_lock_icon(size=14):
    """Charge et met en cache l'icône cadenas redimensionnée."""
    global _LOCK_ICON
...
def _wrap_text(font, text, max_w):
    ...
    Utilisé pour les objectifs de mission dont le texte peut être long.
    """
    words = text.split()
    lines, cur = [], ""
    for w in words:
...
# POPUP — panneau latéral droit d'un chapitre
...
class Popup:
    ...
    def update(self, dt):
        # Lerp vers 1.0 ou 0.0 selon visible — donne un slide fluide
    ...
    def draw(self, screen, sw, sh, save, fonts, hover_start, mx, my):
        """
        Dessine le panneau latéral droit avec les missions du chapitre.
        Retourne le rect absolu du bouton "Commencer" (ou None si pas visible).
        """
        if self._anim < 0.01:
            return None
        ...
        # En-tête : label du chapitre + titre avec retour à la ligne si trop long
        ...
        if ch.get("type") == "cinematique":
            # Pas d'objectifs pour les cinématiques — juste un message et le bouton
            msg  = "Déjà vue." if is_completed else "Cinématique d'introduction."
            note = f_xs.render(msg, True, C_MUTED)
            panel.blit(note, (14, content_y))
            content_y += 22
            no_obj = f_xs.render("Aucun objectif requis.", True, C_MUTED)
            panel.blit(no_obj, (14, content_y))
            ...
            def _mission_row_h(m, locked):
                # Missions verrouillées : hauteur fixe. Sinon on calcule selon les objectifs.
                if locked:
                    return 50
                total = obj_start_y
            ...
            computed_heights = [
                _mission_row_h(m, not is_mission_unlocked(save, self.chapter_idx, i))
            ...
            for i, m in enumerate(missions):
                ...
                # Couleur de fond : sélectionné > normal > verrouillé
                col = (44, 32, 10) if is_sel else ((30, 22, 10) if not locked else (18, 14, 6))
                ...
                if not locked:
                    ...
                    # On lit les états d'objectifs depuis la save — persistants entre sessions
                    ...
                    for oi, obj in enumerate(m.get("objectives", [])):
                        ...
                        lines   = _wrap_text(f_xs, obj["text"], max_obj_w - 16)
                        draw_star(clip_surf, row.x + 8, oy + 1, 12, done_o)
    ...
    def _draw_start_btn(self, surf, font, ph, label, lmx, lmy):
        """Bouton en bas du panneau — doré au survol."""
        bw, bh = self.WIDTH - 24, 34
        br  = pygame.Rect(12, ph - bh - 10, bw, bh)
        hov = br.collidepoint(lmx, lmy)
        pygame.draw.rect(surf, C_GOLD3 if hov else C_GOLD2, br, border_radius=4)
...
# POINTS DE CHAPITRE — cercles animés sur la carte
...
class ChapterPoint:
    ...
    def __init__(self, idx, cx_abs, cy_abs):
        ...
        self.cy      = cy_abs
        self.pulse   = 0.0
        self.ripple  = []   # liste de (progress 0→1, rayon max) pour l'animation de déblocage
        self._appear = 0.0  # 0→1 pour l'animation d'apparition

    def trigger_unlock(self):
        """Lance les cercles concentriques qui s'élargissent au déblocage."""
    ...
    def update(self, dt, unlocked):
        ...
        # On avance chaque ripple et on supprime ceux qui ont terminé
        self.ripple = [(p + dt * 1.2, mr) for p, mr in self.ripple if p + dt * 1.2 < 1.0]
    ...
    def draw(self, surf, unlocked, completed, hovered, save):
        ...
        # Cercles de ripple au déblocage — s'estompent en s'élargissant
        ...
        # Halo pulsant autour du point — plus discret que le ripple
        ...
        # Cercle principal — anneau doré au survol
        if hovered:
        ...
        # Étoile si complété, numéro sinon
...
class Cinematic:
    """
    Affiche les lignes de texte d'une cinématique une par une avec fade in/out.
    Chaque ligne reste 2.8 secondes avant de s'effacer.
    """
    ...
    def draw(self, screen, sw, sh, fonts):
        ...
        # Barres noires en haut et bas — effet cinéma letterbox
        bar_h = sh // 7
        ...
        skip_t = fonts["xs"].render("[ Passer ]", True, (138, 106, 30))
...
class Notification:
    """
    Toast de notification affiché 3 secondes avec fade in/out.
    Utilisé pour signaler un déblocage de chapitre ou de mission.
    """
    ...
    def draw(self, screen, sw, fonts):
        ...
        # Fond semi-transparent avec bordure dorée — même style que les autres panneaux
