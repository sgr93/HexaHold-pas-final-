"""
screens/equipement_screen.py

Écran Équipement : slots autour du perso, loadout de tours, inventaire avec onglets,
popup d'info détaillée (stats, niveau, vente, équiper), tri par rareté.
"""

import os
import pygame
import ui.theme as theme
import core.save_data as sd
from core.config import ALL_TOWER_TYPES, AVAILABLE_TOWERS_INIT, TOWER_SLOT_COUNT, EQUIPMENT_STATS
from ui.ui_screens import ITEM_LABELS


# Slots autour du perso — gauche contient les armures, droite les armes
SLOTS_LEFT  = ["cape", "veste"]
SLOTS_RIGHT = ["arme", "bottes"]

SLOT_LABELS = {
    "cape":   "Cape",
    "veste":  "Veste",
    "bottes": "Bottes",
    "arme":   "Arme",
    "tour":   "Tour",
}

# Noms des fichiers image par slot — utilisés quand l'item n'a pas de champ "image"
EQUIPMENT_IMAGE_FILES = {
    "cape":   "cape.png",
    "veste":  "veste.png",
    "bottes": "bottes.png",
    "arme":   "lames.png",
    "tour":   "tour.png",
}

# Couleurs par rareté — avec accents car c'est ce que config.py utilise
RARITY_COLORS = {
    "Commun":     (120, 120, 128),
    "Rare":       (70,  130, 235),
    "Épique":     (150, 90,  220),
    "Légendaire": (230, 190, 60),
    "Mythique":   (255, 60,  220),
}

# Valeur de vente — on gagne peu sur le commun, beaucoup sur le mythique
SELL_VALUES = {
    "Commun":     25,
    "Rare":       75,
    "Épique":     200,
    "Légendaire": 500,
    "Mythique":   1200,
}

# Ordre de tri pour l'inventaire — 5 = plus rare, 1 = plus commun
RARITY_ORDER = {
    "Mythique":   5,
    "Légendaire": 4,
    "Épique":     3,
    "Rare":       2,
    "Commun":     1,
}

# Descriptions affichées dans le popup quand on clique sur une tour du loadout
TOWER_DESCRIPTIONS = {
    "small":   "Tour légère à tir rapide. Idéale en début de partie pour harceler les ennemis à faible portée.",
    "big":     "Tour lourde au tir lent mais très puissant. Excellente contre les ennemis blindés.",
    "trap":    "Piège au sol qui s'active au passage des ennemis. Invisible jusqu'au contact.",
    "sniper":  "Tour longue portée à dégâts élevés. Cible en priorité l'ennemi le plus avancé.",
    "mortar":  "Lance des obus en zone. Efficace contre les groupes mais lente à recharger.",
    "frost":   "Ralentit les ennemis dans sa zone. Ne tue pas mais facilite le travail des autres tours.",
    "tesla":   "Décharge électrique qui se propage entre ennemis proches. Dévastatrice en groupe.",
    "cannon":  "Canon à forte cadence avec bonne portée. Bon équilibre entre vitesse et puissance.",
    "laser":   "Rayon continu infligeant des dégâts par seconde. Très efficace sur les boss.",
}

# Cache global des sprites — évite de recharger le même fichier à chaque frame
_sprite_cache = {}


def _load_sprite(filename, size=None):
    """Charge un sprite depuis assets/sprites/ et le met en cache par (nom, taille)."""
    if not filename:
        return None
    key = (filename, size)
    if key in _sprite_cache:
        return _sprite_cache[key]
    path = os.path.join(os.path.dirname(__file__), "..", "assets", "sprites", filename)
    try:
        img = pygame.image.load(path).convert_alpha()
        if size:
            img = pygame.transform.smoothscale(img, size)
        _sprite_cache[key] = img
    except Exception:
        _sprite_cache[key] = None
    return _sprite_cache[key]


def _rarity_color(item):
    """Couleur de rareté de l'item — gris neutre si la rareté est inconnue."""
    return RARITY_COLORS.get(item.get("rarity", "Commun"), (120, 120, 128))


def _draw_eq_slot(screen, rect, item, label, f_label, f_lvl):
    """
    Slot d'équipement 90x90 avec bordure colorée selon la rareté.
    Si le slot est vide, affiche juste le label du slot en gris.
    """
    col      = _rarity_color(item) if item else (80, 68, 46)
    border_w = 2 if item else 1

    pygame.draw.rect(screen, (6, 4, 2), rect)
    pygame.draw.rect(screen, col, rect, border_w)

    # Image de l'item ou placeholder vide
    img_name = (item.get("image") or EQUIPMENT_IMAGE_FILES.get(item.get("slot", ""))) if item else None
    img      = _load_sprite(img_name) if img_name else None
    inner    = pygame.Rect(rect.x + 4, rect.y + 4, rect.w - 8, rect.h - 24)

    if img:
        screen.blit(pygame.transform.smoothscale(img, (inner.w, inner.h)), inner.topleft)
    else:
        # Placeholder avec le label du slot centré
        pygame.draw.rect(screen, (20, 15, 8), inner)
        pygame.draw.rect(screen, (60, 48, 28), inner, 1)
        lbl_s = f_label.render(label, True, theme.GOLD_DIM)
        screen.blit(lbl_s, (rect.centerx - lbl_s.get_width() // 2,
                            inner.centery - lbl_s.get_height() // 2))

    # Niveau affiché en bas du slot, ou label si vide
    if item:
        lvl_s = f_lvl.render(f"Niv.{item.get('level', 1)}", True, theme.GOLD_LIGHT)
    else:
        lvl_s = f_lvl.render(label, True, theme.GOLD_DIM)
    screen.blit(lvl_s, (rect.centerx - lvl_s.get_width() // 2,
                        rect.bottom - lvl_s.get_height() - 2))


def _draw_tour_slot(screen, rect, tower_type, f_lvl):
    """
    Slot de tour dans le loadout — même logique que _draw_eq_slot.
    Bordure dorée si occupé, gris si vide.
    """
    col      = (107, 93, 63) if tower_type else (50, 42, 28)
    border_w = 2 if tower_type else 1

    pygame.draw.rect(screen, (6, 4, 2), rect)
    pygame.draw.rect(screen, col, rect, border_w)

    img   = _load_sprite(f"{tower_type}.png") if tower_type else None
    inner = pygame.Rect(rect.x + 4, rect.y + 4, rect.w - 8, rect.h - 20)
    if img:
        screen.blit(pygame.transform.smoothscale(img, (inner.w, inner.h)), inner.topleft)
    else:
        pygame.draw.rect(screen, (20, 15, 8), inner)
        pygame.draw.rect(screen, (50, 42, 28), inner, 1)

    # Nom tronqué à 8 caractères pour tenir dans le slot
    name = ITEM_LABELS.get(tower_type, tower_type or "-")[:8] if tower_type else "-"
    ns   = f_lvl.render(name, True, theme.GOLD_LIGHT if tower_type else theme.GOLD_DIM)
    screen.blit(ns, (rect.centerx - ns.get_width() // 2,
                     rect.bottom - ns.get_height() - 2))


def _draw_panel_popup(screen, rect):
    """Fond sombre avec bordure dorée — base de tous les popups de cet écran."""
    pygame.draw.rect(screen, (18, 14, 8), rect, border_radius=10)
    pygame.draw.rect(screen, theme.GOLD,  rect, 2, border_radius=10)


class EquipementScreen:
    def __init__(self, save):
        self.save                     = save
        self.inv_tab                  = "cape"
        self.selected_loadout_slot    = 0
        # Scroll mémorisé par onglet — le joueur retrouve sa position en changeant d'onglet
        self.scroll_by_tab            = {"cape": 0, "veste": 0, "bottes": 0, "arme": 0, "tour": 0}
        self.popup_open               = False
        self.popup_item_idx           = None
        self.popup_item_source        = None   # "inv" ou "tour"
        self.pending_equip_from_popup = False
        self.pending_sell_idx         = None

    def _get_inventory_item(self, idx):
        """Retourne l'item à l'index donné dans inventory_equipment, ou None si hors bornes."""
        inv = self.save.get("inventory_equipment", [])
        return inv[idx] if 0 <= idx < len(inv) else None

    def _get_item_stats(self, item):
        """
        Calcule les stats de l'item depuis EQUIPMENT_STATS avec le bonus de niveau.
        Chaque niveau ajoute 10% multiplicatif — niveau 1 = base, niveau 5 = base * 1.4.
        """
        slot  = item.get("slot")
        entry = EQUIPMENT_STATS.get(item.get("stats_key") or slot)
        if not entry:
            return {}
        rarity = item.get("rarity", "Commun")
        level  = item.get("level", 1)
        mult   = 1.0 + 0.1 * (level - 1)
        values = entry.get("values", {})
        # Fallback sur Commun si la rareté exacte n'est pas dans les valeurs
        base   = values.get(rarity, values.get("Commun", 0))
        label  = entry.get("label", slot)
        try:
            val = float(base) * mult
        except Exception:
            val = 0.0
        return {label: val}

    def _draw_item_popup(self, screen, item, mx, my, clicked):
        """
        Popup d'info détaillée sur un item.
        Tours : affiche la description textuelle.
        Équipements : affiche les stats calculées avec bonus de niveau.
        Boutons : Équiper (applique direct) et Vendre (crédite des pièces).
        """
        W, H   = 260, 320
        sw, sh = screen.get_size()
        # Centré à l'écran — pas ancré sur le slot pour éviter de déborder sur les bords
        px     = sw // 2 - W // 2
        py     = sh // 2 - H // 2
        rect   = pygame.Rect(px, py, W, H)

        _draw_panel_popup(screen, rect)

        f_title = theme.font(theme.SZ_LABEL, body=True)
        f_txt   = theme.font(theme.SZ_SMALL, body=True)
        f_tiny  = theme.font(theme.SZ_TINY,  body=True)

        # Nom de l'item centré en haut
        name = item.get("name") or SLOT_LABELS.get(item.get("slot"), "Équipement")
        lbl  = f_title.render(name, True, theme.CREAM)
        screen.blit(lbl, (px + W // 2 - lbl.get_width() // 2, py + 6))

        # Rareté colorée juste sous le nom
        rarity = item.get("rarity", "Commun")
        r_col  = RARITY_COLORS.get(rarity, theme.CREAM_DIM)
        r_lbl  = f_tiny.render(rarity, True, r_col)
        screen.blit(r_lbl, (px + W // 2 - r_lbl.get_width() // 2, py + 6 + lbl.get_height()))

        # Image centrée dans la popup
        img_name = f"{item['tower_type']}.png" if item.get("tower_type") else (
            item.get("image") or EQUIPMENT_IMAGE_FILES.get(item.get("slot", ""), "")
        )
        img = _load_sprite(img_name)
        if img:
            screen.blit(pygame.transform.smoothscale(img, (80, 80)), (px + W // 2 - 40, py + 40))

        # Niveau en haut à gauche, loin de l'image pour ne pas se chevaucher
        screen.blit(f_tiny.render(f"Niveau {item.get('level', 1)}", True, theme.GOLD_LIGHT),
                    (px + 12, py + 40))

        # Zone de contenu — description ou stats selon le type d'item
        sy = py + 130
        if item.get("tower_type"):
            # Tours : description textuelle avec retour à la ligne automatique
            desc    = TOWER_DESCRIPTIONS.get(item["tower_type"], "Aucune description disponible.")
            screen.blit(f_txt.render("Description :", True, theme.CREAM), (px + 12, sy))
            sy      += f_txt.get_height() + 6
            words    = desc.split()
            line_buf = []
            max_w    = W - 28
            for word in words:
                test = " ".join(line_buf + [word])
                if f_tiny.size(test)[0] <= max_w:
                    line_buf.append(word)
                else:
                    if line_buf:
                        screen.blit(f_tiny.render(" ".join(line_buf), True, theme.CREAM_DIM), (px + 14, sy))
                        sy += f_tiny.get_height() + 2
                    line_buf = [word]
            if line_buf:
                screen.blit(f_tiny.render(" ".join(line_buf), True, theme.CREAM_DIM), (px + 14, sy))
        else:
            # Équipements : stats avec bonus de niveau appliqué
            stats = self._get_item_stats(item)
            if stats:
                screen.blit(f_txt.render("Statistiques :", True, theme.CREAM), (px + 12, sy))
                sy += f_txt.get_height() + 2
                for k, v in stats.items():
                    v_str = f"{v:.1f}" if isinstance(v, float) else str(v)
                    line  = f_tiny.render(f"{k.capitalize()} : {v_str}", True, theme.CREAM)
                    screen.blit(line, (px + 18, sy))
                    sy += line.get_height() + 1

        # Valeur de vente et bouton vendre — uniquement pour les équipements, pas les tours
        is_tower = bool(item.get("tower_type"))
        if not is_tower:
            sell_val = SELL_VALUES.get(rarity, 10)
            screen.blit(f_tiny.render(f"Valeur de vente : {sell_val} pièces", True, theme.CREAM_DIM),
                        (px + 12, rect.bottom - 70))

        btn_w     = 90
        btn_h     = 28
        # Si c'est une tour, le bouton équiper est centré seul ; sinon il est à gauche
        if is_tower:
            btn_equip = pygame.Rect(px + W // 2 - btn_w // 2, rect.bottom - 40, btn_w, btn_h)
        else:
            btn_equip = pygame.Rect(px + 15,             rect.bottom - 40, btn_w, btn_h)
            btn_sell  = pygame.Rect(px + W - btn_w - 15, rect.bottom - 40, btn_w, btn_h)

        pygame.draw.rect(screen, (40, 70, 40), btn_equip, border_radius=6)
        pygame.draw.rect(screen, theme.GOLD,   btn_equip, 1, border_radius=6)
        screen.blit(f_tiny.render("Équiper", True, theme.CREAM),
                    (btn_equip.centerx - f_tiny.size("Équiper")[0] // 2,
                     btn_equip.centery - f_tiny.get_height() // 2))

        if not is_tower:
            pygame.draw.rect(screen, (80, 40, 40), btn_sell, border_radius=6)
            pygame.draw.rect(screen, theme.GOLD,   btn_sell, 1, border_radius=6)
            screen.blit(f_tiny.render("Vendre", True, theme.CREAM),
                        (btn_sell.centerx - f_tiny.size("Vendre")[0] // 2,
                         btn_sell.centery - f_tiny.get_height() // 2))

        # Croix fermer en haut à droite
        close_rect = pygame.Rect(rect.right - 24, rect.top + 6, 18, 18)
        pygame.draw.rect(screen, (40, 40, 40),   close_rect, border_radius=4)
        pygame.draw.rect(screen, theme.GOLD_DIM, close_rect, 1, border_radius=4)
        x_lbl = f_tiny.render("X", True, theme.CREAM)
        screen.blit(x_lbl, (close_rect.centerx - x_lbl.get_width() // 2,
                            close_rect.centery - x_lbl.get_height() // 2))

        if clicked:
            if close_rect.collidepoint(mx, my):
                self.popup_open        = False
                self.popup_item_idx    = None
                self.popup_item_source = None
            elif btn_equip.collidepoint(mx, my):
                # On ne fait pas l'action ici — on passe par pending pour éviter les effets de bord
                self.pending_equip_from_popup = True
            elif not is_tower and btn_sell.collidepoint(mx, my):
                self.pending_sell_idx = self.popup_item_idx

    def _sell_item(self, idx):
        """
        Vend un item, crédite les pièces et retire l'item de l'inventaire.
        Décale tous les index d'equipped supérieurs à idx pour rester cohérent.
        """
        inv = self.save.get("inventory_equipment", [])
        if not (0 <= idx < len(inv)):
            return
        rarity             = inv[idx].get("rarity", "Commun")
        self.save["coins"] = self.save.get("coins", 0) + SELL_VALUES.get(rarity, 10)

        eqp = self.save.get("equipped", {})
        # Déséquiper si l'item était dans un slot
        for k, v in list(eqp.items()):
            if v == idx:
                eqp[k] = None
        # Décaler les index supérieurs — sinon les autres slots pointent vers le mauvais item
        for k, v in list(eqp.items()):
            if isinstance(v, int) and v > idx:
                eqp[k] = v - 1

        inv.pop(idx)
        self.save["inventory_equipment"] = inv
        self.save["equipped"]            = eqp
        sd.save(self.save)

    def _auto_equip_item(self, idx):
        """
        Équipe l'item dans son slot naturel (cape -> slot cape, etc.).
        Écrase l'item précédent sans confirmation — c'est voulu, le joueur peut re-équiper.
        """
        inv = self.save.get("inventory_equipment", [])
        if not (0 <= idx < len(inv)):
            return
        slot = inv[idx].get("slot")
        if not slot:
            return
        eqp       = self.save.get("equipped", {})
        eqp[slot] = idx
        self.save["equipped"] = eqp
        sd.save(self.save)

    def _equip_tower_to_loadout(self, tower_type, slot_idx):
        """
        Place une tour dans le slot du loadout sélectionné.
        Le loadout est normalisé à TOWER_SLOT_COUNT entrées à chaque appel.
        """
        if not tower_type or not (0 <= slot_idx < TOWER_SLOT_COUNT):
            return
        tl = self.save.get("tower_loadout", list(AVAILABLE_TOWERS_INIT[:TOWER_SLOT_COUNT]))
        if not isinstance(tl, list):
            tl = list(AVAILABLE_TOWERS_INIT[:TOWER_SLOT_COUNT])
        while len(tl) < TOWER_SLOT_COUNT:
            tl.append(None)
        tl[slot_idx]               = tower_type
        self.save["tower_loadout"] = tl[:TOWER_SLOT_COUNT]
        sd.save(self.save)

    def draw(self, screen, area, mx, my, clicked, scroll_dy=0):
        save  = self.save
        f_sec = theme.font(theme.SZ_SECTION)
        f_sm  = theme.font(theme.SZ_SMALL, body=True)
        f_ti  = theme.font(theme.SZ_TINY,  body=True)

        # Quand le popup est ouvert, les clics sur le fond sont bloqués
        clicked_bg = clicked and not self.popup_open

        pad = 10
        x   = area.x + pad
        y   = area.y + pad
        w   = area.width - pad * 2

        theme.render_text(screen, "Équipement", f_sec, theme.GOLD_LIGHT, x, y)
        theme.draw_gold_rule(screen, x, y + f_sec.get_height() + 2, w)
        y += f_sec.get_height() + 10

        # PANNEAU PERSO — fond maison1.png avec overlay sombre + portrait d'Eren centré
        panel_h = 370
        panel   = pygame.Rect(x, y, w, panel_h)

        bg = _load_sprite("maison1.png")
        if bg:
            screen.blit(pygame.transform.smoothscale(bg, (panel.w, panel.h)), panel.topleft)
            # Overlay pour assombrir le fond et faire ressortir les slots
            ov = pygame.Surface((panel.w, panel.h), pygame.SRCALPHA)
            ov.fill((10, 7, 4, 70))
            screen.blit(ov, panel.topleft)
        else:
            pygame.draw.rect(screen, theme.DARK_2, panel)
        pygame.draw.rect(screen, theme.GOLD, panel, 2)

        # Portrait d'Eren — ratio conservé, hauteur fixée à 280px
        eren   = _load_sprite("eren.png")
        eren_w = 160
        eren_h = 280
        if eren:
            scale  = eren_h / eren.get_height()
            eren_w = int(eren.get_width() * scale)
            screen.blit(pygame.transform.smoothscale(eren, (eren_w, eren_h)),
                        (panel.centerx - eren_w // 2, panel.y + 8))
        else:
            # Fallback rectangle si le sprite est absent
            ph = pygame.Rect(panel.centerx - 55, panel.y + 8, 110, 170)
            pygame.draw.rect(screen, theme.DARK_3,   ph)
            pygame.draw.rect(screen, theme.GOLD_DIM, ph, 1)

        inv = save.get("inventory_equipment", [])
        eqp = save.get("equipped", {})

        SLOT_W = 90
        SLOT_H = 90
        GAP    = 6
        offset = 80  # distance horizontale entre Eren et les slots

        # Slots gauche (cape, veste)
        left_x = panel.centerx - eren_w // 2 - SLOT_W - offset
        for i, slot in enumerate(SLOTS_LEFT):
            sy   = panel.y + 8 + i * (SLOT_H + GAP)
            sr   = pygame.Rect(left_x, sy, SLOT_W, SLOT_H)
            idx  = eqp.get(slot)
            item = inv[idx] if isinstance(idx, int) and 0 <= idx < len(inv) else None
            _draw_eq_slot(screen, sr, item, SLOT_LABELS[slot], f_ti, f_ti)
            if clicked_bg and sr.collidepoint(mx, my) and item:
                self.popup_open        = True
                self.popup_item_idx    = idx
                self.popup_item_source = "inv"

        # Slots droite (arme, bottes)
        right_x = panel.centerx + eren_w // 2 + offset
        for i, slot in enumerate(SLOTS_RIGHT):
            sy   = panel.y + 8 + i * (SLOT_H + GAP)
            sr   = pygame.Rect(right_x, sy, SLOT_W, SLOT_H)
            idx  = eqp.get(slot)
            item = inv[idx] if isinstance(idx, int) and 0 <= idx < len(inv) else None
            _draw_eq_slot(screen, sr, item, SLOT_LABELS[slot], f_ti, f_ti)
            if clicked_bg and sr.collidepoint(mx, my) and item:
                self.popup_open        = True
                self.popup_item_idx    = idx
                self.popup_item_source = "inv"

        # Loadout de tours centré en bas du panneau
        tower_loadout = save.get("tower_loadout", AVAILABLE_TOWERS_INIT[:TOWER_SLOT_COUNT])
        if not isinstance(tower_loadout, list):
            tower_loadout = list(AVAILABLE_TOWERS_INIT[:TOWER_SLOT_COUNT])
        while len(tower_loadout) < TOWER_SLOT_COUNT:
            tower_loadout.append(None)
        tower_loadout = tower_loadout[:TOWER_SLOT_COUNT]

        T_W           = 80
        T_H           = 80
        T_GAP         = 14
        total_tours_w = TOWER_SLOT_COUNT * T_W + (TOWER_SLOT_COUNT - 1) * T_GAP
        tx_start      = panel.centerx - total_tours_w // 2
        ty            = panel.bottom - T_H - 10

        tl = f_sm.render("Tours", True, theme.CREAM)
        screen.blit(tl, (panel.centerx - tl.get_width() // 2, ty - tl.get_height() - 4))

        for i, tt in enumerate(tower_loadout):
            tr     = pygame.Rect(tx_start + i * (T_W + T_GAP), ty, T_W, T_H)
            is_sel = i == self.selected_loadout_slot
            _draw_tour_slot(screen, tr, tt, f_ti)
            # Surbrillance dorée sur le slot sélectionné
            if is_sel:
                pygame.draw.rect(screen, theme.GOLD_LIGHT, tr, 2)
            if clicked_bg and tr.collidepoint(mx, my):
                # Clic sur un slot de tour = sélection uniquement, pas de popup
                self.selected_loadout_slot = i

        y += panel_h + 8

        # ONGLETS INVENTAIRE
        tab_labels = [
            ("cape",   "Cape"),
            ("veste",  "Veste"),
            ("bottes", "Bottes"),
            ("arme",   "Arme"),
            ("tour",   "Tours"),
        ]
        tab_w_each = (w - (len(tab_labels) - 1) * 4) // len(tab_labels)
        cx2        = x
        for key, label in tab_labels:
            tr2    = pygame.Rect(cx2, y, tab_w_each, 30)
            is_act = self.inv_tab == key
            theme.draw_panel(screen, tr2,
                             color=(30, 22, 8) if is_act else theme.DARK_2,
                             border_color=theme.GOLD if is_act else theme.GOLD_DIM,
                             radius=theme.RADIUS_SM, border_w=2 if is_act else 1)
            theme.render_text(screen, label, f_ti,
                              theme.GOLD_LIGHT if is_act else theme.CREAM_DIM,
                              tr2.centerx, tr2.centery - f_ti.get_height() // 2,
                              center=True, shadow=False)
            if clicked_bg and tr2.collidepoint(mx, my):
                self.inv_tab = key
                self.scroll_by_tab.setdefault(key, 0)
            cx2 += tab_w_each + 4

        y += 36

        # Filtrage de l'inventaire selon l'onglet actif
        if self.inv_tab == "tour":
            # Onglet tours : on liste les tours débloquées du joueur
            tu    = save.get("towers_unlocked", {})
            tl_s  = save.get("towers_level", {})
            items = [
                (i, {"slot":       "tour",
                     "rarity":     sd.TOWER_POOL.get(tt, {}).get("rarity", "Commun"),
                     "tower_type": tt,
                     "level":      tl_s.get(tt, 1)})
                for i, tt in enumerate(ALL_TOWER_TYPES) if tu.get(tt, False)
            ]
        else:
            # Autres onglets : items de l'inventaire filtrés par slot
            items = [(i, it) for i, it in enumerate(inv) if it.get("slot") == self.inv_tab]

        # Tri par rareté — les items les plus rares en premier pour les trouver vite
        items.sort(key=lambda e: RARITY_ORDER.get(e[1].get("rarity", "Commun"), 1), reverse=True)

        # Grille scrollable — on clip pour ne pas déborder hors de la zone
        COLS       = 10
        CELL       = w // COLS
        clip       = pygame.Rect(x, y, w, area.bottom - y)
        screen.set_clip(clip)

        self.scroll_by_tab[self.inv_tab] = max(
            0, self.scroll_by_tab.get(self.inv_tab, 0) - scroll_dy * 20
        )
        cur_scroll = self.scroll_by_tab[self.inv_tab]

        for idx, (orig_idx, item) in enumerate(items):
            col     = idx % COLS
            row     = idx // COLS
            cx3     = x + col * CELL
            cy3     = y + row * CELL - cur_scroll
            cr      = pygame.Rect(cx3 + 1, cy3 + 1, CELL - 3, CELL - 3)
            # Skip les cellules hors de la zone visible — optimisation simple mais efficace
            if cr.bottom < clip.top or cr.top > clip.bottom:
                continue

            rc = _rarity_color(item)
            pygame.draw.rect(screen, (14, 10, 6), cr)
            pygame.draw.rect(screen, rc, cr, 2)

            img_name = (f"{item.get('tower_type', '')}.png" if self.inv_tab == "tour"
                        else item.get("image") or EQUIPMENT_IMAGE_FILES.get(item.get("slot", "")))
            img    = _load_sprite(img_name) if img_name else None
            inner2 = pygame.Rect(cr.x + 3, cr.y + 3, cr.w - 6, cr.h - 16)
            if img:
                screen.blit(pygame.transform.smoothscale(img, (inner2.w, inner2.h)), inner2.topleft)
            else:
                pygame.draw.rect(screen, (20, 15, 8), inner2)

            lvl_s = f_ti.render(f"Nv.{item.get('level', 1)}", True, theme.CREAM)
            screen.blit(lvl_s, (cr.centerx - lvl_s.get_width() // 2,
                                cr.bottom - lvl_s.get_height() - 1))

            # Badge "E" doré si l'item est actuellement équipé dans un slot
            if self.inv_tab != "tour" and orig_idx in list(eqp.values()):
                es = f_ti.render("E", True, theme.DARK)
                eb = pygame.Rect(cr.x + 1, cr.y + 1, es.get_width() + 3, es.get_height() + 1)
                pygame.draw.rect(screen, theme.GOLD_LIGHT, eb)
                screen.blit(es, (eb.x + 1, eb.y))

            if clicked_bg and cr.collidepoint(mx, my):
                self.popup_open        = True
                self.popup_item_idx    = orig_idx
                self.popup_item_source = "tour" if self.inv_tab == "tour" else "inv"

        screen.set_clip(None)

        # Limiter le scroll au contenu réel pour éviter de scroller dans le vide
        rows   = (len(items) + COLS - 1) // COLS
        max_sc = max(0, rows * CELL - (area.bottom - y))
        self.scroll_by_tab[self.inv_tab] = min(self.scroll_by_tab[self.inv_tab], max_sc)

        # POPUP — rendu en dernier pour passer par-dessus tout le reste
        if self.popup_open and self.popup_item_idx is not None:
            if self.popup_item_source == "inv":
                item = self._get_inventory_item(self.popup_item_idx)
            elif self.popup_item_source == "tour":
                # Vérification des bornes avant d'accéder à ALL_TOWER_TYPES
                if 0 <= self.popup_item_idx < len(ALL_TOWER_TYPES):
                    tt   = ALL_TOWER_TYPES[self.popup_item_idx]
                    item = {
                        "slot":       "tour",
                        "tower_type": tt,
                        "rarity":     sd.TOWER_POOL.get(tt, {}).get("rarity", "Commun"),
                        "level":      save.get("towers_level", {}).get(tt, 1),
                        "name":       tt,
                    }
                else:
                    item = None
            else:
                item = None
            if item:
                self._draw_item_popup(screen, item, mx, my, clicked)

        # Actions déclenchées depuis le popup — traitées après le rendu pour éviter les conflits
        if self.pending_equip_from_popup and self.popup_item_idx is not None:
            if self.popup_item_source == "inv":
                self._auto_equip_item(self.popup_item_idx)
            elif self.popup_item_source == "tour" and 0 <= self.popup_item_idx < len(ALL_TOWER_TYPES):
                self._equip_tower_to_loadout(
                    ALL_TOWER_TYPES[self.popup_item_idx], self.selected_loadout_slot
                )
            self.pending_equip_from_popup = False
            self.popup_open               = False
            self.popup_item_idx           = None
            self.popup_item_source        = None

        if self.pending_sell_idx is not None and self.popup_item_source == "inv":
            self._sell_item(self.pending_sell_idx)
            self.pending_sell_idx  = None
            self.popup_open        = False
            self.popup_item_idx    = None
            self.popup_item_source = None

        return None